...
\passthrough{\lstinline!assignment02/!})，题面细节以文件头注释为准；FROM-SCRATCH
题。

硬件与构建:主线环境是 B300，\passthrough{\lstinline!cuda/Makefile!} 默认
\passthrough{\lstinline!ARCH=100f!}；M0、M1 也可以在 5090
上完成(\passthrough{\lstinline!ARCH=120a make ...!})； M2 的判测纯
host，无卡可判；M3、M4、M5 需要 B300。本作业统一用显式
\passthrough{\lstinline!-gencode!} 而不用
\passthrough{\lstinline!-arch=sm\_XXXa!} 简写，原因见
\passthrough{\lstinline!assignment02/README.md!}，Makefile 已经配好。

实验纪律:所有性能数字都在自己占满的卡上测；测前测后
\passthrough{\lstinline!nvidia-smi --query-compute-apps=pid，name --format=csv!}
查有没有别的 进程占卡；可能 hang 的程序套
\passthrough{\lstinline!timeout -k 5 <秒数>!}(判测脚本已带)。

AI政策:必做题沿用仓库根目录 \passthrough{\lstinline!CLAUDE.md!}------AI
可以帮你理解、 review，不能替你实现；团队题不设限制。详见
\passthrough{\lstinline!assignment02/README.md!}。
...
\passthrough{\lstinline!cuda/m0\_env/!} \\
1 & sm80:fragment 与 mma.sync & P2.2(S025--S041) &
\passthrough{\lstinline!cuda/m1\_sm80/!} \\
2 & smem 供数:descriptor 与 swizzle & P2.3(S042--S070) &
\passthrough{\lstinline!cuda/m2\_smem/!} \\
3 & sm100:tcgen05 & P2.4(S071--S093) &
\passthrough{\lstinline!cuda/m3\_tcgen05/!} \\
4 & 完整 GEMM 四步走 & S084 + Session 4 &
\passthrough{\lstinline!cuda/m4\_gemm/!} \\
5 & 低精度与 block scaling & P3(S094--S111) &
\passthrough{\lstinline!cuda/m5\_lowprec/!}、\passthrough{\lstinline!kernels/!} \\
6 & TileLang 对照 & 收尾(S112) & (复用 assignment01) \\
Team & FlashKDA / MSA decode & --- & \passthrough{\lstinline!team/!} \\
\end{longtable}

\hypertarget{ux73afux5883ux4e0eux5cf0ux503c}{%
...
datasheet 对照值与口径差异 & 5090:fp4差异巨大,来源于专用fp4路径 &
同5090 \\
...
\hypertarget{sm80fragment-ux4e0e-mma.sync}{%
...
一节；fp8 转换用
\passthrough{\lstinline!cuda\_fp8.h!}(\passthrough{\lstinline!\_\_nv\_fp8\_e4m3!})。
...
\passthrough{\lstinline!mma.sync.aligned.m16n8k32.row.col.f32.e4m3.e4m3.f32!}
...
在 \passthrough{\lstinline!cuda/m1\_sm80/03\_mma\_fp8.cu!}
中从零实现一个单 tile 的 fp8 mma，不提供代码骨架。使用
\passthrough{\lstinline!m16n8k32!} e4m3 mma、f32 累加，并手动完成
fragment 装载（本题不使用 ldmatrix），最后与 CPU
参考结果进行严格相等比较。
...
程序接受一个 seed 参数，例如 \passthrough{\lstinline!./prog 123!}；

输出必须以 \passthrough{\lstinline!PASS!} 或
\passthrough{\lstinline!MISMATCH!} / \passthrough{\lstinline!FAIL!}
开头；

fp8 与 float 的互转直接使用 \passthrough{\lstinline!cuda\_fp8.h!}。
...
\passthrough{\lstinline!ldmatrix!}
的装载路径。两种实现需要共存，并分别通过判测。

根据 PTX 文档选择合适的 \passthrough{\lstinline!ldmatrix!}
变体（\passthrough{\lstinline!.x1/.x2/.x4!}，以及是否使用
\passthrough{\lstinline!.trans!}）。B 矩阵在 shared memory
中的布局还需要满足 \passthrough{\lstinline!ldmatrix!} 的 16 byte
行地址要求，具体约束见文件头说明。

两条路径都通过判测后，使用
\passthrough{\lstinline!make ptx/m1\_sm80/04\_ldmatrix!} 或
\passthrough{\lstinline!nvdisasm!} 查看生成的指令，分别统计
\passthrough{\lstinline!smem → fragment!}
阶段的装载指令和地址计算指令数量，并回答：

\begin{enumerate}
\def\labelenumi{(\alph{enumi})}
\item
  \passthrough{\lstinline!ldmatrix!} 省掉了手工装载中的哪些工作？
\item
  为什么这些工作在手工装载路径中无法避免？
\end{enumerate}
...
观察行跨度对 \passthrough{\lstinline!ldmatrix!} 的影响。对同一个 16×16
fp16 tile，分别使用 32 B、64 B、128 B 和 128+16 B（padding）四种行跨度。

先根据课上介绍的 bank 模型，预测四种情况下执行一次
\passthrough{\lstinline!ldmatrix!} 所需的 wavefront
数及其比例，再运行程序并使用 Nsight Compute 测量：
...
的 fragment。1.3 手动完成每个 lane 的装载，1.4 使用
\passthrough{\lstinline!ldmatrix!} 简化这一步，而 1.5 说明即使使用
\passthrough{\lstinline!ldmatrix!}，shared memory 的 bank conflict
仍然会影响实际效率。
...
\hypertarget{smem-ux4f9bux6570descriptor-ux4e0e-swizzle}{%
...
\passthrough{\lstinline!wgmma.mma\_async!} /
\passthrough{\lstinline!st.shared!} /
\passthrough{\lstinline!wgmma.commit\_group!} /
\passthrough{\lstinline!fence.proxy.async!} /
\passthrough{\lstinline!wgmma.fence!} /
\passthrough{\lstinline!wgmma.wait\_group!}
...
\tightlist
\item
  \passthrough{\lstinline!fence.proxy.async!} 是 wgmma 专属的指令，TMA
  与 tcgen05 的场景不需要它。
\item
  \passthrough{\lstinline!wgmma.commit\_group!} 会阻塞，直到它之前发射的
  wgmma 全部完成。
\item
  不加 \passthrough{\lstinline!fence.proxy.async!} 时，wgmma 可能读到
  shared memory 中的旧值，因为 \passthrough{\lstinline!st.shared!}
...
本题实现的 \passthrough{\lstinline!swizzle\_128B!} 会在 3.2 中直接用于
...
\def\labelenumi{(\alph{enumi})}
\item
  \passthrough{\lstinline!tcgen05.ld!} 读取 TMEM 时，每个 warp
  只能读取自己对应的 32 条 lane，warp 之间不能互相读取。
\item
  与 \passthrough{\lstinline!mma.sync!} 由 warp 协作、wgmma 由 warpgroup
  协作不同，\passthrough{\lstinline!tcgen05.mma!}
  由单个线程发射，随后由硬件异步执行。
\item
...
\item
  \passthrough{\lstinline!tcgen05.commit!} 会阻塞直到之前发射的 mma
...
f32 累加、\passthrough{\lstinline!cta\_group::1!}，并由一个 128 线程的
block 完成。

数据按照下面的路径流动：

\passthrough{\lstinline!global → smem（K-major + 128B swizzle）→ tcgen05.mma → TMEM → tcgen05.ld → global!}

代码骨架只保留课件 F27 中的七步流程注释。descriptor 使用 2.2
中实现的编码方式，shared memory staging 使用 2.3 中的
\passthrough{\lstinline!swizzle\_128B!}。TMEM alloc、mbarrier、idesc
以及 mma / ld 的具体写法需要根据 PTX ISA 完成，相关语义可参考课件
C15--C21。

\passthrough{\lstinline!fence.proxy.async!} 的位置以及
\passthrough{\lstinline!tcgen05.ld!} 的 warp
可见范围已经在文件头中给出。
...
在正确版本通过后，故意去掉 \passthrough{\lstinline!fence.proxy.async!}
...
\item
  分别记录
  \passthrough{\lstinline!rounds = 1!}、\passthrough{\lstinline!2!}、\passthrough{\lstinline!4!}
  时的运行结果以及问题出现的条件。
\item
...
提示：注意 \passthrough{\lstinline!tcgen05.ld!} 与尚未完成的 mma
之间的关系。

\prob{3.4}{EXPERIMENT}[cuda/m3\_tcgen05/04\_cta\_pair.cu]

比较 \passthrough{\lstinline!cta\_group::1!} 和
\passthrough{\lstinline!cta\_group::2!} 完成同一个 m256n64k64
任务时的数据开销。

\passthrough{\lstinline!cta\_group::1!} 使用两个独立
block；\passthrough{\lstinline!cta\_group::2!} 使用一个 cluster 发射一条
M=256 的 mma，两个 CTA 分别保存 B 矩阵的一部分。
...
\def\labelenumi{(\alph{enumi})}
\item
  在 \passthrough{\lstinline!cta\_group::2!} 中，每个 CTA 所需的 B
  shared memory 是 \passthrough{\lstinline!cta\_group::1!} 的多少？TMEM
  的占用又如何变化？程序会打印 shared memory 用量，用它验证你的预测。
\item
  使用 Nsight Compute 比较两种实现的 shared memory 总流量。
\item
  \passthrough{\lstinline!cta\_group::2!} 节省下来的 shared memory
  容量，在 M4 的 pipeline 中可以用来做什么？
\item
  \passthrough{\lstinline!cta\_group::2!} 依赖 sm90
  引入的哪一种硬件机制？5090 不支持 2-CTA MMA，结合 0.2
...
\passthrough{\lstinline!cp.async.bulk.tensor!}、mbarrier（\passthrough{\lstinline!expect\_tx!}）小节；CUDA
Driver API 的 \passthrough{\lstinline!cuTensorMapEncodeTiled!}。
...
K 维循环完成累加。数据 staging 仍然使用
\passthrough{\lstinline!st.shared!} 和 swizzle。
...
通过判测后，填写性能表中 \passthrough{\lstinline!4.1 tiled!} 一行。结合
0.2 中计算的机器 平衡点，判断此时性能主要受哪一环节限制。

\prob{4.2}{MODIFY}[cuda/m4\_gemm/02\_tma.cu]

将 4.1 中的 shared memory staging 改为 TMA。

host 端使用 \passthrough{\lstinline!cuTensorMapEncodeTiled!} 创建 tensor
map；kernel 中使用 \passthrough{\lstinline!cp.async.bulk.tensor!} 搭配
mbarrier 的 \passthrough{\lstinline!expect\_tx!} 完成搬运。
本题仍然使用单缓冲。
...
通过判测后，填写性能表中 \passthrough{\lstinline!4.2 TMA!} 一行。
...
将 4.2 的单缓冲改为 \passthrough{\lstinline!STAGES!} 级循环缓冲，使后续
K tile 的 TMA 预取能够与当前 tile 的 mma 计算重叠。

\passthrough{\lstinline!STAGES!} 为编译参数，例如：

\begin{lstlisting}
STAGES=4 make -B run/m4_gemm/03_pipeline
\end{lstlisting}

这里的 \passthrough{\lstinline!-B!} 不能省略，否则修改
\passthrough{\lstinline!STAGES!} 后可能不会重新编译。
...
\def\labelenumi{\arabic{enumi}.}
\item
  使用 \passthrough{\lstinline!S=3!} 的结果填写性能表中
  \passthrough{\lstinline!4.3 pipeline!} 一行。
\item
  完成不同 stage 数的性能测试：
  ...
  比较两个形状对 \passthrough{\lstinline!STAGES!} 的敏感程度，并结合
  shared memory 用量、 每个 SM 可同时驻留的 block 数以及 block
  间并发能够隐藏的延迟进行解释。
\item
  任选一个 \passthrough{\lstinline!STAGES!}，画出稳态阶段各 stage 中 TMA
  与 mma 的流水时空图。
\item
  回答：
...
\def\labelenumi{(\alph{enumi})}
\item
  实现 4.3 的 \passthrough{\lstinline!cta\_group::2!} 版本，利用 B
  shared memory 用量的减少 增加 pipeline 深度或扩大 tile；
\item
  在 5090 上使用 \passthrough{\lstinline!mma.sync!} 实现相同的
  tiling，并与 B300 的结果比较；
\item
...
\passthrough{\lstinline!vllm/models/kimi\_k3/nvidia/low\_latency\_gemm.py!}
...
\item
  ...
  哪些形状主要受到显存带宽限制？
\item
  \passthrough{\lstinline!f\_b\_proj!}（K=128）中两个 roof
...
\passthrough{\lstinline!in\_proj\_qkvgfab!} 对应 KDA
...
\hypertarget{ux4f4eux7cbeux5ea6ux4e0e-block-scaling}{%
\section{低精度与 block
...
S108）；\passthrough{\lstinline!cuda\_fp4.h!} 中的
\passthrough{\lstinline!\_\_nv\_fp4x2\_e2m1!}；cuBLASLt 的 block-scaled
FP4 matmul。格式约定见题面材料
\passthrough{\lstinline!cuda/m5\_lowprec/nvfp4\_common.h!}。
...
\tightlist
\item
  \passthrough{\lstinline!gemm\_scale\_per\_row\_col!}:A 每行一个
  scale，B 每个输出列一个 scale。scale
  乘积在整个点积中不变，完整归约后只用乘回一次。
\item
  \passthrough{\lstinline!gemm\_scale\_along\_k!}:A 和 B 的 scale 每 128
  个 K 元素改变 一次。每个 K block 的 partial sum 要分别乘回该段的 scale
  乘积， 然后进行累加。
\end{itemize}

文件还提供了错误范例
\passthrough{\lstinline!gemm\_scale\_along\_k\_one\_restore!}：它先把
...
\passthrough{\lstinline!nvfp4\_common.h!} 中给出的格式约定：
...
\item
  SF 使用 \passthrough{\lstinline!amax / 6!}，并以 e4m3 保存；
\item
  SF 按 Tensor Core 消费要求的 swizzled 布局存放，字节偏移公式已经给出。
\end{itemize}

\hypertarget{a-e2m1-ux7f16ux7801}{%
\paragraph{(a) E2M1 编码}\label{a-e2m1-ux7f16ux7801}}

在 \passthrough{\lstinline!e2m1\_encode.h!} 中实现 host/device 通用的
E2M1 round-to-nearest-even 编码器。

判测会在 GPU 上使用 \passthrough{\lstinline!cuda\_fp4.h!} 中的
\passthrough{\lstinline!\_\_nv\_fp4x2\_e2m1!}
...
\hypertarget{b-nvfp4-quant-kernel}{%
\paragraph{(b) NVFP4 quant kernel}\label{b-nvfp4-quant-kernel}}

在 \passthrough{\lstinline!nvfp4\_quant\_kernel.h!} 中实现 quant
kernel，完成

\passthrough{\lstinline!bf16 → e2m1 打包 + e4m3 SF + swizzled SF 布局!}
...
正确实现下，FP4 matmul 的 \passthrough{\lstinline!maxrel!} 约为
\passthrough{\lstinline!4e-3!}，主要来自 bf16 输出的舍入误差。如果 SF
布局错位或 scale 对应到了错误的量化组，
通常会出现成块的明显数值错误，而不是仅有小幅舍入误差。

\hypertarget{c-ceiling-probe}{%
\paragraph{(c) Ceiling probe}\label{c-ceiling-probe}}

在 \passthrough{\lstinline!03c\_ceiling\_probe.cu!} 中实现一个 ceiling
probe。它与 quant kernel
...
\hypertarget{d-optional}{%
\paragraph{(d) Optional}\label{d-optional}}

使用 tcgen05 \passthrough{\lstinline!kind::mxf4nvf4!} 直接消费自己生成的
NVFP4 数据， SF 通过 TMEM 提供，并与 cuBLASLt 的结果对拍。

\begin{lookback}

5.3(c) 用来区分量化 kernel 中的访存成本和数值转换成本。作为参考，
软件实现 E2M1 RN-even 时，每个 kernel 约执行 18129 条 FSETP；
使用硬件转换后约为 1272 条 \passthrough{\lstinline!F2FP.E2M1!}，同一
kernel 的时间从 71.7 µs 降到 32.8 µs。Nsight Compute 中的瓶颈也从 SM
利用率 84.7\% 的 compute-bound 转为 memory-bound。
...
实现融合的 \passthrough{\lstinline!rms\_norm + NVFP4!} kernel：
...
\hypertarget{tilelang-ux5bf9ux7167}{%
...
\passthrough{\lstinline!kernels/tilelang\_matmul.py!}）；TileLang
...
使用 assignment01 的
\passthrough{\lstinline!kernels/tilelang\_matmul.py!}，或任意使用
\passthrough{\lstinline!T.gemm!} 的 kernel，分别以
\passthrough{\lstinline!sm\_90a!} 和 \passthrough{\lstinline!sm\_100a!}
...
\passthrough{\lstinline!Tensor Core 指令选择与供数布局!}

\hypertarget{ux56e2ux961fux9009ux505aux63a8ux8350}{%
\section*{团队选做（推荐）}\label{ux56e2ux961fux9009ux505aux63a8ux8350}}
\addcontentsline{toc}{section}{团队选做（推荐）}

2--4 人一组，从 \passthrough{\lstinline!team/!}
中的两个题目任选一个，也可以全部完成：
...
\passthrough{\lstinline!复现与测量 → 分析（结论 + 证据）→ 挑战!}

挑战阶段不要求一定得到正向加速。如果实验结果表明继续优化收益有限，
只要能够用数据说明为什么不值得继续做，同样视为有效结论。

答辩时间为 10 分钟，另有 5 分钟提问；提问优先由选择另一道团队题的
小组提出。具体要求见 \passthrough{\lstinline!team/README.md!}。

团队题不限制 AI 工具的使用。

\hypertarget{ux63d0ux4ea4}{%
...
\item
  ...
  \passthrough{\lstinline!team/README.md!}。
\end{itemize}
